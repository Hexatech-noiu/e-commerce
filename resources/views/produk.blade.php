...
<section class="bg-center bg-no-repeat bg-white bg-blend-multiply mt-10 md:mt-32">
    ...
    <div class="max-w-3xl mx-auto px-8 sm:px-0">
        <div class="flex justify-center sm:mx-auto">
            <div role="tablist" aria-label="tabs" class="relative w-max mx-auto h-12 grid grid-cols-6 items-center px-[3px] rounded-sm bg-gray-900/20 overflow-hidden shadow-2xl shadow-900/20 transition">
                ...
                <a href="{{route('produk_many'). '?category='. urlencode($cat->name)}}" role="tab" aria-selected="true" aria-controls="{{$cat->name}}" id="tab-1" tabindex="0" class="{{urldecode(request('category')) == $cat->name ? 'bg-white rounded-sm' : ''}} text-center self-center relative block h-10 px-6 tab rounded-sm">
    ...
    {{-- Produk --}}
    <div class="flex items-center justify-center tab-panel p-6 transition duration-300">
        <div role="tabpanel" id="panel-1" class="grid grid-cols-1 gap-5 md:grid-cols-3 lg:grid-cols-4">
            ...
            <div class="rounded-sm group relative h-72 items-center justify-center overflow-hidden transition-shadow hover:shadow-xl hover:shadow-black/30">
                <div class="h-full w-72">
                    <img class="h-full w-full object-cover transition-transform duration-500 group-hover:rotate-3 group-hover:scale-125" src="{{ Storage::url($p->picture) }}" alt="{{$p->name}}" />
                </div>
                ...
                <div class="absolute inset-0 flex translate-y-[50%] flex-col items-center justify-center px-9 text-center transition-all duration-500 group-hover:translate-y-0">
                    <h1 class="font-dmserif text-3xl font-bold text-white">{{$p->name}}</h1>

                    <p class="mb-3 text-lg text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">Rp. {{$p->price}}</p>

                    <div class="flex justify-center items-center gap-2">
                        <a href="#" class="rounded-sm bg-secondary-40 py-2 px-3.5 text-sm capitalize text-black hover:text-white hover:bg-primary-40 duration-[400ms] shadow shadow-black/60">Beli Produk</a>
                        <a href="#" class="rounded-sm bg-transparent py-1.5 px-2.5 text-sm capitalize text-secondary-40 border-2 border-secondary-40 hover:text-black hover:bg-secondary-40 duration-[400ms] shadow shadow-black/60"><i class="fa fa-eye"></i></a>
                    </div>
                </div>
            </div>
            @endforeach
...
<div class="flex flex-wrap justify-start right-0 lg:mx-0">
    ...
    <div class="max-w-xs bg-white border border-neutral-20 rounded-lg shadow mx-auto md:mx-0 mb-5 md:ml-9">
        <div class="rounded-t-lg w-[318px] h-[229px] overflow-hidden">
            <img class="object-cover w-full h-full transform origin-center hover:scale-125 transition-transform duration-500 cursor-pointer" src="{{ Storage::url($pkg->picture) }}" loading="eager" />
        </div>
        <div class="p-5">
            ...
            <p class="mb-2 text-lg font-semibold tracking-tight text-primary-40">Rp.
                {{ number_format($pkg->price, 0, ',', '.') }}<span class="text-sm font-medium text-neutral-60"> /Pax</span></p>
            <button type="button" id="btnSeeDetail{{ $pkg->id }}" onclick="ViewModal({{ $pkg->id }})" data-category="{{ $tpkg->name }}" data-name="{{ $pkg->name }}" data-picture="{{ Storage::url($pkg->picture) }}" data-price="{{ number_format($pkg->price, 0, ',', '.') }}" data-desc="{!! $pkg->desc !!}" data-modal-target="seeDetailModal" data-modal-toggle="seeDetailModal" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-primary-40 rounded-lg hover:text-black hover:bg-secondary-40 focus:ring-4 focus:outline-none focus:ring-secondary-20 duration-[400ms]">
                See Detail
