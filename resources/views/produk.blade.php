...
@section('container')
    ...
    <section class="bg-center bg-no-repeat bg-white bg-blend-multiply mt-10 md:mt-32">
        <div class="px-4 mx-auto max-w-screen-xl text-center">
            <h1 class="mb-10 text-2xl font-extrabold tracking-tight leading-none text-black sm:text-5xl">
                Produk
            </h1>
        </div>
        ...
        <div class="max-w-3xl mx-auto px-8 sm:px-0">
            <div class="flex justify-center sm:mx-auto">
                <div role="tablist" aria-label="tabs"
                    class="relative w-max mx-auto flex flex-wrap max-w-screen-xl items-center p-[4px] rounded-sm bg-gray-900/20 overflow-hidden shadow-2xl shadow-900/20 transition">
                    <div class="absolute indicator h-10 my-auto top-0 bottom-0 left-0 rounded-sm bg-white shadow-md"></div>
                    <button role="tab" aria-selected="true" onclick="getData(0, '#panel-1')" aria-controls="panel-1"
                        id="tab-1" tabindex="0" class="relative block h-10 px-6 tab rounded-sm">
                        <span class="flex items-center justify-center text-center text-gray-800">Semua</span>
                    </button>
                    @foreach ($categories as $cat)
                        <button role="tab" aria-selected="true" onclick="getData({{ $cat->id }}, '#panel-1')"
                            ...
                            <span class="flex items-center text-gray-800">{{ $cat->name }}</span>
                        </button>
                    @endforeach
                </div>
            </div>
