                    ...
                    @foreach ($cat->products as $prod)
                        <div
                            ...
                            <div class="h-full w-72">
                                <img class="h-full w-full flex justify-center object-cover transition-transform duration-500 group-hover:rotate-3 group-hover:scale-125"
                            ...
                            <div
                                ...
                                <div class="flex justify-center items-center gap-2"
                                    id="actionWrapper{{ $prod->id }}" data-picture="{{ $prod->picture }}"
                                    data-name="{{ $prod->name }}" data-desc="{{ $prod->desc }}"
                                    data-price="{{ $prod->price }}"
                                    data-fixed-price="{{ number_format($prod->price, 0, ',', '.') }}"
                                    data-category-id="{{ $prod->category->id }}"
                                    data-category-name="{{ $prod->category->name }}">
                                    ...
                                    <a href="/preview/{{ $prod->id }}" data-modal-toggle="readProdukModal"
                                        onclick="copyProp({{ $prod->id }}, 'read')"
